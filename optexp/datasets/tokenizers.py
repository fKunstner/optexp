...
class Tokenizer(ABC, Component):
    ...
    @abstractmethod
    def build_tokenizer(self, model_path: Path, data_path: Path, vocab_size: int):
        raise NotImplementedError()

    @abstractmethod
    def tokenize_and_numify(self, model_path: Path, data_path: Path):
        raise NotImplementedError()
...
class BPETokenizer(Tokenizer):
    ...
    def tokenize_and_numify(self, model_path: Path, data_path: Path):
        ...
        success = tokenizer.load(model_path.absolute().as_posix())
        if not success:
